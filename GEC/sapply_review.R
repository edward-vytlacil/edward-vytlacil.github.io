...
# https://edward-vytlacil.github.io/GEC/PROGRESA_Handout_GEC.html
# reviewing use of sapply.  It builds upon
# what you learned  about functions in R from functions_R.R
# which you can download from
# https://edward-vytlacil.github.io/GEC/functions_R.R
...
class(df$r_M)
class(df$r_fbalx)
# and so forth for all 12 variablesa with 12 lines of code, or 
# we could use sapply:
sapply(df,class)
# notice how class(df) is a data frame, while sapply(df,class) returns the class
# of each varable in the data frame.
...
is.numeric(df$date)
is.numeric(df$r_M)
is.numeric(df$r_fbalx)
# and so forth for all 12 variables, or more convenient:
